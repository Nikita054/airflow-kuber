...
AIRFLOW_IMAGE=nkotov64/airflow-kub
AIRFLOW_TAG=latest
...
do
  ...
  PODS=$(kubectl get pods -n airflow-kuber | awk 'NR>1 {print $0}')
  echo "$PODS"
  NUM_AIRFLOW_READY=$(echo $PODS | grep airflow | wc -l | xargs)
  NUM_POSTGRES_READY=$(echo $PODS | grep postgres | wc -l | xargs)
